...
# 星の指標についての初心者向け解説
with st.expander("⭐ 「星の指標（AI感情分析）」の見かた"):
    st.write("""
    この指標は、AIが最新ニュースから「投資家の心理状態」を分析したものです。
    - **5.0 (⭐⭐⭐⭐⭐)**: 非常に明るいニュースが多く、期待が高まっています。
    - **3.0 (⭐⭐⭐)**: 中立、または良いニュースと悪いニュースが混在しています。
    - **1.0 (⭐)**: 懸念材料や厳しいニュースが多く、警戒が必要です。
    """)
...
if st.button("🚀 AI診断スタート"):
    ...
    with st.spinner('AIが市場の歴史と感情を分析しています...'):
...
st.markdown("""<div class="disclaimer-box"><strong>【免責事項】</strong><br>● 予想額は過去のトレンドに基づくシミュレーションであり、将来の成果を保証するものではありません。● 「星の指標」はAIによる感情分析結果であり、投資勧誘ではありません。● 最終的な投資判断は必ずご自身で行ってください。[PR]アフィリエイト報酬を得る場合があります。</div>""", unsafe_allow_html=True)
